...
def update_teacher_view(request,pk):
    try:
        ...
        messages.error(request, f'Teacher with ID {pk} does not exist.')
        return HttpResponseRedirect('/admin-view-teacher')
    except TMODEL.User.DoesNotExist:
        messages.error(request, f'User account for teacher {pk} does not exist.')
        return HttpResponseRedirect('/admin-view-teacher')
...
def delete_teacher_view(request,pk):
    try:
        ...
        user.delete()
        teacher.delete()
        messages.success(request, f'Teacher {teacher.get_name()} has been deleted successfully.')
        return HttpResponseRedirect('/admin-view-teacher')
    except TMODEL.Teacher.DoesNotExist:
        messages.error(request, f'Teacher with ID {pk} does not exist.')
        return HttpResponseRedirect('/admin-view-teacher')
...
def reject_teacher_view(request,pk):
    try:
        ...
        user.delete()
        teacher.delete()
        messages.success(request, f'Teacher {teacher.get_name()} has been rejected and removed.')
        return HttpResponseRedirect('/admin-view-pending-teacher')
    except TMODEL.Teacher.DoesNotExist:
        messages.error(request, f'Teacher with ID {pk} does not exist.')
        return HttpResponseRedirect('/admin-view-pending-teacher')
...
def update_student_view(request,pk):
    ...
    except SMODEL.User.DoesNotExist:
        messages.error(request, f'User account for student {pk} does not exist.')
        return HttpResponseRedirect('/admin-view-student')
...
def delete_student_view(request,pk):
    try:
        ...
        user.delete()
        student.delete()
        messages.success(request, f'Student {student.get_name()} has been deleted successfully.')
        return HttpResponseRedirect('/admin-view-student')
    except SMODEL.Student.DoesNotExist:
        messages.error(request, f'Student with ID {pk} does not exist.')
        return HttpResponseRedirect('/admin-view-student')
...
def delete_course_view(request,pk):
    try:
        ...
        messages.error(request, f'Course with ID {pk} does not exist.')
        return HttpResponseRedirect('/admin-view-course')
...
def admin_view_question_view(request):
    courses= models.Course.objects.all()
    # Calculate actual question counts for each course
...
def delete_question_view(request,pk):
    try:
        ...
        messages.error(request, f'Question with ID {pk} does not exist.')
        return HttpResponseRedirect('/admin-view-question')
...
@login_required(login_url='adminlogin')
def admin_view_marks_view(request,pk=None):
    courses = models.Course.objects.all()
    # Calculate actual question counts and participant counts for each course
    for course in courses:
        course.actual_question_count = models.Question.objects.filter(course=course).count()
        course.participant_count = models.Result.objects.filter(exam=course).count()
...
@login_required(login_url='adminlogin')
def admin_view_student_marks_detail_view(request, pk):
    try:
        student = SMODEL.Student.objects.get(id=pk)
        results = models.Result.objects.filter(student=student)
        return render(request,'exam/admin_check_marks.html',{'results':results, 'student':student})
    except SMODEL.Student.DoesNotExist:
        # Redirect to student marks page with error message
        messages.error(request, f'Student with ID {pk} does not exist.')
        return HttpResponseRedirect('/admin-view-student-marks')
...
def admin_check_marks_view(request,pk):
    try:
        ...
        messages.error(request, f'Course with ID {pk} does not exist.')
        return HttpResponseRedirect('/admin-view-marks')
    except SMODEL.Student.DoesNotExist:
